...
********* TINY FILE DIALOGS OFFICIAL WEBSITE IS ON SOURCEFORGE *********
 ...
 | | | the windows only wchar_t UTF-16 prototypes are at the bottom of the
header file |
 |_________________________________________________________________________________|
...
#ifndef _GNU_SOURCE
#define _GNU_SOURCE /* used only to resolve symbolic links. Can be commented   \
                       out */
#ifndef _POSIX_C_SOURCE
#ifdef __FreeBSD__
#define _POSIX_C_SOURCE                                                        \
    199506L /* 199506L is enough for freebsd for realpath() */
#elif defined(__illumos__) || defined(__solaris__)
#define _POSIX_C_SOURCE                                                        \
    200112L /* illumos/solaris needs 200112L for realpath() */
#else
...
/******************************************************************************************************/
#ifdef _WIN32
/* if you want to use UTF-8 ( instead of the UTF-16/wchar_t functions at the end
of tinyfiledialogs.h ) Make sure your code is really prepared for UTF-8 (on
windows, functions like fopen() expect MBCS and not UTF-8) */
int tinyfd_winUtf8 =
    1; /* on windows char strings can be 1:UTF-8(default) or 0:MBCS */
...
int tinyfd_silent = 1;  /* 1 (default) or 0 : on unix, hide errors and warnings
                           from called dialogs */

/* Curses dialogs are difficult to use, on windows they are only ascii and uses
 * the unix backslah */
...
/* some systems don't set the environment variable DISPLAY even when a graphic
display is present. set this to 1 to tell tinyfiledialogs to assume the
existence of a graphic display */
...
#ifdef _MSC_VER
#pragma warning(disable : 4996) /* allows usage of strncpy, strcpy, strcat,    \
                                   sprintf, fopen */
#pragma warning(disable : 4100) /* allows usage of strncpy, strcpy, strcat,    \
                                   sprintf, fopen */
#pragma warning(disable : 4706) /* allows usage of strncpy, strcpy, strcat,    \
                                   sprintf, fopen */
#endif
...
static char* getLastName(char* aoDestination, /* make sure it is allocated */
                         char const* aSource) {
...
static void Hex2RGB(char const aHexRGB[8], unsigned char aoResultRGB[3]) {
    ...
    if (aoResultRGB) {
        if (aHexRGB) {
            ...
            aoResultRGB[2] =
                (unsigned char) strtoul(lColorChannel + 5, nullptr, 16);
            lColorChannel[5] = '\0';
            aoResultRGB[1] =
                (unsigned char) strtoul(lColorChannel + 3, nullptr, 16);
            lColorChannel[3] = '\0';
            aoResultRGB[0] =
                (unsigned char) strtoul(lColorChannel + 1, nullptr, 16);
            /* printf("%d %d %d\n", aoResultRGB[0], aoResultRGB[1],
             * aoResultRGB[2]); */
        } else {
            aoResultRGB[0] = 0;
            aoResultRGB[1] = 0;
...
static void RGB2Hex(unsigned char const aRGB[3], char aoResultHexRGB[8]) {
    ...
    defined(__clang__)
            sprintf(aoResultHexRGB, "#%02hhx%02hhx%02hhx", aRGB[0], aRGB[1],
                    aRGB[2]);
#else
            sprintf(aoResultHexRGB, "#%02hx%02hx%02hx", aRGB[0], aRGB[1],
                    aRGB[2]);
#endif
...
void tfd_replaceSubStr(char const* aSource, char const* aOldSubStr,
    ...
    p = aSource;
    *aoDestination = '\0';
    while ((pOccurence = strstr(p, aOldSubStr)) != nullptr) {
...
char const* tinyfd_getGlobalChar(
    char const* aCharVariableName) /* to be called from C# (you don't need this
                                      in C or C++) */
{
    if (!aCharVariableName || !strlen(aCharVariableName)) return nullptr;
...
int tinyfd_getGlobalInt(
    char const* aIntVariableName) /* to be called from C# (you don't need this
                                     in C or C++) */
{
...
int tinyfd_setGlobalInt(
    ...
    int aValue) /* to be called from C# (you don't need this in C or C++) */
{
    if (!aIntVariableName || !strlen(aIntVariableName)) return -1;
...
#ifdef _WIN32
static int
powershellPresent(void) { /*only on vista and above (or installed on xp)*/
    ...
    if (lPowershellPresent < 0) {
        ...
        while (fgets(lBuff, sizeof(lBuff), lIn) != nullptr) {}
        _pclose(lIn);
...
static int sizeUtf16From8(char const* aUtf8string) {
    return MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, aUtf8string, -1,
                               nullptr, 0);
}

static int sizeUtf16FromMbcs(char const* aMbcsString) {
    return MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, aMbcsString, -1,
                               nullptr, 0);
}

static int sizeUtf8(wchar_t const* aUtf16string) {
    return WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, aUtf16string, -1,
                               nullptr, 0, nullptr, nullptr);
}

static int sizeMbcs(wchar_t const* aMbcsString) {
    int lRes = WideCharToMultiByte(CP_ACP, 0, aMbcsString, -1, nullptr, 0,
                                   nullptr, nullptr);
...
wchar_t* tinyfd_mbcsTo16(char const* aMbcsString) {
    static wchar_t* lMbcsString = nullptr;
    int lSize;
...
wchar_t* tinyfd_utf8to16(char const* aUtf8string) {
    static wchar_t* lUtf16string = nullptr;
    int lSize;

    free(lUtf16string);
    if (!aUtf8string) {
        lUtf16string = nullptr;
        return nullptr;
    }
    lSize = sizeUtf16From8(aUtf8string);
    if (lSize) {
        lUtf16string = (wchar_t*) malloc(lSize * sizeof(wchar_t));
        lSize = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, aUtf8string,
                                    -1, lUtf16string, lSize);
        return lUtf16string;
    } else {
...
char* tinyfd_utf16toMbcs(wchar_t const* aUtf16string) {
    static char* lMbcsString = nullptr;
    int lSize;
    ...
    if (lSize) {
        ...
        lSize = WideCharToMultiByte(CP_ACP, 0, aUtf16string, -1, lMbcsString,
                                    lSize, nullptr, nullptr);
...
char* tinyfd_utf16to8(wchar_t const* aUtf16string) {
    static char* lUtf8string = nullptr;
    int lSize;
    ...
    if (lSize) {
        ...
        lSize = WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, aUtf16string,
                                    -1, lUtf8string, lSize, nullptr, nullptr);
...
static void wipefileW(wchar_t const* aFilename) {
    int i;
    FILE* lIn;
#if (defined(__MINGW32_MAJOR_VERSION) && !defined(__MINGW64__) &&              \
...
static wchar_t*
getLastNameW(wchar_t* aoDestination, /* make sure it is allocated */
...
static void Hex2RGBW(wchar_t const aHexRGB[8], unsigned char aoResultRGB[3]) {
    ...
    if (aoResultRGB) {
        if (aHexRGB) {
            ...
            aoResultRGB[2] =
                (unsigned char) wcstoul(lColorChannel + 5, nullptr, 16);
            lColorChannel[5] = '\0';
            aoResultRGB[1] =
                (unsigned char) wcstoul(lColorChannel + 3, nullptr, 16);
            lColorChannel[3] = '\0';
            aoResultRGB[0] =
                (unsigned char) wcstoul(lColorChannel + 1, nullptr, 16);
            /* printf("%d %d %d\n", aoResultRGB[0], aoResultRGB[1],
             * aoResultRGB[2]); */
        } else {
            aoResultRGB[0] = 0;
...
#if !defined(__BORLANDC__) && !defined(__TINYC__) &&                           \
    !(defined(__MINGW32__) && !defined(__MINGW64_VERSION_MAJOR))
            swprintf(aoResultHexRGB, 8, lPrintFormat, aRGB[0], aRGB[1],
                     aRGB[2]);
#else
...
static int dirExists(char const* aDirPath) {
    ...
    if (!lDirLen) return 1;
    if ((lDirLen == 2) && (aDirPath[1] == ':')) return 1;
    ...
    defined(__BORLANDC__) || defined(__WATCOMC__)
        ...
        else if (lInfo.st_mode & S_IFDIR) return 1;
        else return 0;
    }
#if (defined(__MINGW32_MAJOR_VERSION) && !defined(__MINGW64__) &&              \
     (__MINGW32_MAJOR_VERSION <= 3)) ||                                        \
...
static int fileExists(char const* aFilePathAndName) {
    ...
    defined(__BORLANDC__) || defined(__WATCOMC__)
        ...
        if (lStatRet != 0) return 0;
        else if (lInfo.st_mode & _S_IFREG) return 1;
        else return 0;
    } else {
        lIn = fopen(aFilePathAndName, "r");
        if (!lIn) {
...
static void replaceWchar(wchar_t* aString, wchar_t aOldChr, wchar_t aNewChr) {
    wchar_t* p;
...
static void hiddenConsoleW(wchar_t const* aString, wchar_t const* aDialogTitle,
                           int aInFront) {
    ...
    if (!CreateProcessW(nullptr, (LPWSTR) aString, nullptr, nullptr, FALSE,
...
int tinyfd_messageBoxW(
    wchar_t const* aTitle,      /* nullptr or "" */
    wchar_t const* aMessage,    /* nullptr or ""  may contain \n and \t */
    wchar_t const* aDialogType, /* "ok" "okcancel" "yesno" "yesnocancel" */
    wchar_t const* aIconType,   /* "info" "warning" "error" "question" */
    int aDefaultButton)         /* 0 for cancel/no , 1 for ok/yes , 2 for no in
                                   yesnocancel */
{
    int lBoxReturnValue;
    UINT aCode;
    ...
    /*if (quoteDetectedW(aTitle)) return tinyfd_messageBoxW(L"INVALID TITLE WITH
    QUOTES", aMessage, aDialogType, aIconType, aDefaultButton); if
    (quoteDetectedW(aMessage)) return tinyfd_messageBoxW(aTitle, L"INVALID
    MESSAGE WITH QUOTES", aDialogType, aIconType, aDefaultButton);*/
    ...
    lBoxReturnValue =
        MessageBoxW(GetForegroundWindow(), aMessage, aTitle, aCode);

    if ((lBoxReturnValue == IDNO) &&
        (aDialogType && !wcscmp(L"yesnocancel", aDialogType))) {
        return 2;
                ...
                wchar_t const * aIconType)
{
                ...
                if (aTitle && !wcscmp(aTitle, L"tinyfd_query")) {
strcpy(tinyfd_response, "windows_wchar"); return 1; }

                if (quoteDetectedW(aTitle)) return tinyfd_notifyPopupW(L"INVALID
TITLE WITH QUOTES", aMessage, aIconType); if (quoteDetectedW(aMessage)) return
tinyfd_notifyPopupW(aTitle, L"INVALID MESSAGE WITH QUOTES", aIconType);

                lTitleLen = aTitle ? wcslen(aTitle) : 0;
                lMessageLen = aMessage ? wcslen(aMessage) : 0;
                lDialogStringLen = 3 * MAX_PATH_OR_CMD + lTitleLen +
lMessageLen; lDialogString = (wchar_t *) malloc(2 * lDialogStringLen); if
(!lDialogString) return 0;

                wcscpy(lDialogString, L"powershell.exe -executionpolicy bypass
-command \"\
    ...
    wchar_t const* aIconType) /* L"info" L"warning" L"error" */
{
    ...
    if (quoteDetectedW(aTitle))
        return tinyfd_notifyPopupW(L"INVALID TITLE WITH QUOTES", aMessage,
                                   aIconType);
    if (quoteDetectedW(aMessage))
        return tinyfd_notifyPopupW(aTitle, L"INVALID MESSAGE WITH QUOTES",
                                   aIconType);
...
");
    ...
    wcscpy(lDialogString, L"cmd.exe /c mshta.exe \"%TEMP%\\tinyfd.hta\"");
...
wchar_t* tinyfd_inputBoxW(
    ...
    wchar_t const* aDefaultInput) /* L"" , if nullptr it's a passwordBox */
{
    ...
    if (quoteDetectedW(aTitle))
        return tinyfd_inputBoxW(L"INVALID TITLE WITH QUOTES", aMessage,
                                aDefaultInput);
    if (quoteDetectedW(aMessage))
        return tinyfd_inputBoxW(aTitle, L"INVALID MESSAGE WITH QUOTES",
                                aDefaultInput);
    if (quoteDetectedW(aDefaultInput))
        return tinyfd_inputBoxW(aTitle, aMessage,
                                L"INVALID DEFAULT_INPUT WITH QUOTES: use the "
                                L"GRAVE ACCENT \\x60 instead.");
...
<input type = 'button' value = 'Cancel' name = 'btn_Cancel' onClick = 'vbs:Run_ProgramCancel' style = 'width: 5em;'><br><br>\n\
        ...
        wcscat(
            lDialogString,
            L"<table width = '100%' height = '100%' align = 'center' border = '0'>\n\
<tr>\n\
...
wchar_t* tinyfd_saveFileDialogW(
    wchar_t const* aTitle,                   /* nullptr or "" */
    wchar_t const* aDefaultPathAndOrFile,    /* nullptr or "" */
    int aNumOfFilterPatterns,                /* 0 */
    wchar_t const* const* aFilterPatterns,   /* nullptr or {"*.jpg","*.png"} */
    wchar_t const* aSingleFilterDescription) /* nullptr or "image files" */
{
    static wchar_t lBuff[MAX_PATH_OR_CMD];
    ...
    /*if (quoteDetectedW(aTitle)) return tinyfd_saveFileDialogW(L"INVALID TITLE
    WITH QUOTES", aDefaultPathAndOrFile, aNumOfFilterPatterns, aFilterPatterns,
    aSingleFilterDescription); if (quoteDetectedW(aDefaultPathAndOrFile)) return
    tinyfd_saveFileDialogW(aTitle, L"INVALID DEFAULT_PATH WITH QUOTES",
    aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription); if
    (quoteDetectedW(aSingleFilterDescription)) return
    tinyfd_saveFileDialogW(aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns,
    aFilterPatterns, L"INVALID FILTER_DESCRIPTION WITH QUOTES"); for (i = 0; i <
    aNumOfFilterPatterns; i++)
    {
                    if (quoteDetectedW(aFilterPatterns[i])) return
    tinyfd_saveFileDialogW(L"INVALID FILTER_PATTERN WITH QUOTES: use the GRAVE
    ACCENT \\x60 instead.", aDefaultPathAndOrFile, 0, nullptr, nullptr);
    }*/

    lHResult = CoInitializeEx(nullptr, 0);
    ...
    ofn.hInstance = 0;
    ofn.lpstrFilter = wcslen(lFilterPatterns) ? lFilterPatterns : nullptr;
    ...
    ofn.lpstrInitialDir = wcslen(lDirname) ? lDirname : nullptr;
    ofn.lpstrTitle = aTitle && wcslen(aTitle) ? aTitle : nullptr;
...
wchar_t* tinyfd_openFileDialogW(
    ...
    wchar_t const* const* aFilterPatterns,   /* nullptr or {"*.jpg","*.png"} */
    wchar_t const* aSingleFilterDescription, /* nullptr or "image files" */
    int aAllowMultipleSelects) /* 0 or 1 ; -1 to free allocated memory and
                                  return */
{
    ...
    /*if (quoteDetectedW(aTitle)) return tinyfd_openFileDialogW(L"INVALID TITLE
    WITH QUOTES", aDefaultPathAndOrFile, aNumOfFilterPatterns, aFilterPatterns,
    aSingleFilterDescription, aAllowMultipleSelects); if
    (quoteDetectedW(aDefaultPathAndOrFile)) return
    tinyfd_openFileDialogW(aTitle, L"INVALID DEFAULT_PATH WITH QUOTES",
    aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription,
    aAllowMultipleSelects); if (quoteDetectedW(aSingleFilterDescription)) return
    tinyfd_openFileDialogW(aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns,
    aFilterPatterns, L"INVALID FILTER_DESCRIPTION WITH QUOTES",
    aAllowMultipleSelects); for (i = 0; i < aNumOfFilterPatterns; i++)
    {
                    if (quoteDetectedW(aFilterPatterns[i])) return
    tinyfd_openFileDialogW(L"INVALID FILTER_PATTERN WITH QUOTES: use the GRAVE
    ACCENT \\x60 instead.", aDefaultPathAndOrFile, 0, nullptr, nullptr,
    ...
    ofn.lpstrTitle = aTitle && wcslen(aTitle) ? aTitle : nullptr;
    ofn.Flags =
        OFN_EXPLORER | OFN_NOCHANGEDIR | OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST;
    ofn.nFileOffset = 0;
    ofn.nFileExtension = 0;
    ...
    if (GetOpenFileNameW(&ofn) == 0) {
        ...
        if (lBuff)
            lBuff =
                (wchar_t*) (realloc(lBuff, (lBuffLen + 1) * sizeof(wchar_t)));
    }
...
static int __stdcall BrowseCallbackProcW(HWND hwnd, UINT uMsg, LPARAM lp,
                                         LPARAM pData) {
    (void) lp;
    switch (uMsg) {
...
wchar_t*
tinyfd_selectFolderDialogW(wchar_t const* aTitle,       /* nullptr or "" */
                           wchar_t const* aDefaultPath) /* nullptr or "" */
{
    static wchar_t lBuff[MAX_PATH_OR_CMD];
    wchar_t* lRetval;
    ...
    /*if (quoteDetectedW(aTitle)) return tinyfd_selectFolderDialogW(L"INVALID
    TITLE WITH QUOTES", aDefaultPath); if (quoteDetectedW(aDefaultPath)) return
    tinyfd_selectFolderDialogW(aTitle, L"INVALID DEFAULT_PATH WITH QUOTES");*/

    lHResult = CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED);

    bInfo.hwndOwner = GetForegroundWindow();
    bInfo.pidlRoot = nullptr;
    bInfo.pszDisplayName = lBuff;
    bInfo.lpszTitle = aTitle && wcslen(aTitle) ? aTitle : nullptr;
    if (lHResult == S_OK || lHResult == S_FALSE) {
...
wchar_t*
tinyfd_colorChooserW(wchar_t const* aTitle,         /* nullptr or "" */
                     wchar_t const* aDefaultHexRGB, /* nullptr or "#FF0000"*/
                     ...
                     unsigned char aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
    ...
    /*if (quoteDetectedW(aTitle)) return tinyfd_colorChooserW(L"INVALID TITLE
    WITH QUOTES", aDefaultHexRGB, aDefaultRGB, aoResultRGB); if
    (quoteDetectedW(aDefaultHexRGB)) return tinyfd_colorChooserW(aTitle,
    L"INVALID DEFAULT_HEX_RGB WITH QUOTES: use the GRAVE ACCENT \\x60 instead.",
                                ...
                                yesnocancel */
{
    ...
    lIntRetVal = tinyfd_messageBoxW(lTitle, lMessage, lDialogType, lIconType,
                                    aDefaultButton);
...
static int
notifyWinGui(char const* aTitle,   /* nullptr or "" */
             char const* aMessage, /* nullptr or "" may NOT contain \n nor \t */
...
static int inputBoxWinGui(
    char* aoBuff, char const* aTitle, /* nullptr or "" */
    char const* aMessage,      /* nullptr or "" may NOT contain \n nor \t */
    char const* aDefaultInput) /* "" , if nullptr it's a passwordBox */
{
    wchar_t lTitle[128] = L"";
    wchar_t* lMessage = nullptr;
    ...
    } else lTmpWChar = tinyfd_inputBoxW(lTitle, lMessage, nullptr);
...
static char* saveFileDialogWinGui(
    char* aoBuff, char const* aTitle,     /* nullptr or "" */
    char const* aDefaultPathAndOrFile,    /* nullptr or "" */
    int aNumOfFilterPatterns,             /* 0 */
    char const* const* aFilterPatterns,   /* nullptr or {"*.jpg","*.png"} */
    char const* aSingleFilterDescription) /* nullptr or "image files" */
{
    ...
    lTmpWChar = tinyfd_saveFileDialogW(
        lTitle, lDefaultPathAndFile, aNumOfFilterPatterns,
    ...
    strcpy(aoBuff, lTmpChar);
    if (tinyfd_winUtf8) (void) tinyfd_utf16to8(nullptr);
    else (void) tinyfd_utf16toMbcs(nullptr);
    ...
    int aAllowMultipleSelects)            /* 0 or 1 */
{
    ...
    if (!lTmpWChar) return nullptr;

    if (tinyfd_winUtf8) lTmpChar = tinyfd_utf16to8(lTmpWChar);
    else lTmpChar = tinyfd_utf16toMbcs(lTmpWChar);
    (void) tinyfd_openFileDialogW(nullptr, nullptr, 0, nullptr, nullptr, -1);

    return lTmpChar;
}

static char*
selectFolderDialogWinGui(char* aoBuff, char const* aTitle, /*  nullptr or "" */
...
static char*
colorChooserWinGui(char const* aTitle,                 /* nullptr or "" */
                   char const* aDefaultHexRGB,         /* nullptr or "#FF0000"*/
                   unsigned char const aDefaultRGB[3], /* { 0 , 255 , 255 } */
                   unsigned char aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
    ...
    lTmpWChar =
        tinyfd_colorChooserW(lTitle, lDefaultHexRGB, aDefaultRGB, aoResultRGB);
                                ...
                                yesnocancel */
{
    ...
    if (aDialogType &&
        (!strcmp("okcancel", aDialogType) || !strcmp("yesno", aDialogType) ||
    ...
    char const* aDefaultInput) /* "" , if nullptr it's a passwordBox */
{
    ...
    while (fgets(aoBuff, MAX_PATH_OR_CMD, lIn) != nullptr) {}
    fclose(lIn);
...
static char*
saveFileDialogWinConsole(char* aoBuff, char const* aTitle,  /* nullptr or "" */
                         char const* aDefaultPathAndOrFile) /* nullptr or "" */
{
    char lDialogString[MAX_PATH_OR_CMD];
    ...
    strcat(lDialogString, "--backtitle \"");
    strcat(lDialogString, "tab: focus | /: populate | spacebar: fill text "
                          "field | ok: TEXT FIELD ONLY");
    strcat(lDialogString, "\" ");
...
static char*
openFileDialogWinConsole(char const* aTitle,                /*  nullptr or "" */
                         char const* aDefaultPathAndOrFile) /*  nullptr or "" */
{
...
static void writeUtf8(char const* aUtf8String) {
    ...
    (void) WriteConsoleW(lConsoleHandle, lTmpWChar, (DWORD) wcslen(lTmpWChar),
                         &lNum, nullptr);
}
                                ...
                                yesnocancel */
{
    ...
    if (tfd_quoteDetected(aTitle))
        return tinyfd_messageBox("INVALID TITLE WITH QUOTES", aMessage,
                                 aDialogType, aIconType, aDefaultButton);
    if (tfd_quoteDetected(aMessage))
        return tinyfd_messageBox(aTitle, "INVALID MESSAGE WITH QUOTES",
                                 aDialogType, aIconType, aDefaultButton);
    ...
    if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent())) &&
        ...
        return messageBoxWinGui(aTitle, aMessage, aDialogType, aIconType,
                                aDefaultButton);
        ...
        return messageBoxWinConsole(aTitle, aMessage, aDialogType, aIconType,
                                    aDefaultButton);
    } else {
        if (!tinyfd_winUtf8) {
    ...
    char const* aIconType) /* "info" "warning" "error" */
{
    if (tfd_quoteDetected(aTitle))
        return tinyfd_notifyPopup("INVALID TITLE WITH QUOTES", aMessage,
                                  aIconType);
    if (tfd_quoteDetected(aMessage))
        return tinyfd_notifyPopup(aTitle, "INVALID MESSAGE WITH QUOTES",
                                  aIconType);
    ...
    char const* aDefaultInput) /* "" , if nullptr it's a passwordBox */
{
    ...
    if (tfd_quoteDetected(aTitle))
        return tinyfd_inputBox("INVALID TITLE WITH QUOTES", aMessage,
                               aDefaultInput);
    if (tfd_quoteDetected(aMessage))
        return tinyfd_inputBox(aTitle, "INVALID MESSAGE WITH QUOTES",
                               aDefaultInput);
    if (tfd_quoteDetected(aDefaultInput))
        return tinyfd_inputBox(aTitle, aMessage,
                               "INVALID DEFAULT_INPUT WITH QUOTES: use the "
                               "GRAVE ACCENT \\x60 instead.");
    ...
    if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent())) &&
        ...
        if (tinyfd_winUtf8) {
            ...
            (void) ReadConsoleW(lConsoleHandle, lBuffW, MAX_PATH_OR_CMD, &lNum,
                                nullptr);
            if (!aDefaultInput) {
    ...
    char const* aSingleFilterDescription) /* nullptr or "image files" */
{
    ...
    if (tfd_quoteDetected(aTitle))
        return tinyfd_saveFileDialog(
            "INVALID TITLE WITH QUOTES", aDefaultPathAndOrFile,
            aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription);
    if (tfd_quoteDetected(aDefaultPathAndOrFile))
        return tinyfd_saveFileDialog(aTitle, "INVALID DEFAULT_PATH WITH QUOTES",
                                     aNumOfFilterPatterns, aFilterPatterns,
                                     aSingleFilterDescription);
    if (tfd_quoteDetected(aSingleFilterDescription))
        return tinyfd_saveFileDialog(aTitle, aDefaultPathAndOrFile,
                                     aNumOfFilterPatterns, aFilterPatterns,
    ...
    for (i = 0; i < aNumOfFilterPatterns; i++) {
        if (tfd_quoteDetected(aFilterPatterns[i]))
            return tinyfd_saveFileDialog("INVALID FILTER_PATTERN WITH QUOTES: "
                                         ...
                                         aDefaultPathAndOrFile, 0, nullptr,
                                         nullptr);
    }
    ...
    if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent())) &&
        ...
        p = saveFileDialogWinGui(
            lBuff, aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns,
        ...
        lPointerInputBox = tinyfd_inputBox(
            nullptr, nullptr, nullptr); /* obtain a pointer on the current
        ...
        if (lPointerInputBox)
            strcpy(
                lPointerInputBox,
                lString); /* restore its previous content to tinyfd_inputBox */
        p = lBuff;
    }
    ...
    int aAllowMultipleSelects)            /* 0 or 1 */
{
    ...
    if (tfd_quoteDetected(aDefaultPathAndOrFile))
        return tinyfd_openFileDialog(
            aTitle, "INVALID DEFAULT_PATH WITH QUOTES", aNumOfFilterPatterns,
            aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects);
    if (tfd_quoteDetected(aSingleFilterDescription))
        return tinyfd_openFileDialog(aTitle, aDefaultPathAndOrFile,
                                     aNumOfFilterPatterns, aFilterPatterns,
                                     "INVALID FILTER_DESCRIPTION WITH QUOTES",
                                     aAllowMultipleSelects);
    for (i = 0; i < aNumOfFilterPatterns; i++) {
    ...
    if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent())) &&
        ...
        p = openFileDialogWinGui(
            aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns,
            (char const* const*) aFilterPatterns, aSingleFilterDescription,
            aAllowMultipleSelects);
    } else if (dialogPresent()) {
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
                                ...
                                char const* aDefaultPath) /* nullptr or "" */
{
    ...
    if (tfd_quoteDetected(aTitle))
        return tinyfd_selectFolderDialog("INVALID TITLE WITH QUOTES",
                                         aDefaultPath);
    if (tfd_quoteDetected(aDefaultPath))
        return tinyfd_selectFolderDialog(aTitle,
    ...
    unsigned char aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
    ...
    if (tfd_quoteDetected(aTitle))
        return tinyfd_colorChooser("INVALID TITLE WITH QUOTES", aDefaultHexRGB,
                                   aDefaultRGB, aoResultRGB);
    if (tfd_quoteDetected(aDefaultHexRGB))
        return tinyfd_colorChooser(aTitle,
                                   "INVALID DEFAULT_HEX_RGB WITH QUOTES: use "
                                   "the GRAVE ACCENT \\x60 instead.",
                                   aDefaultRGB, aoResultRGB);
    ...
    lPointerInputBox = tinyfd_inputBox(
        nullptr, nullptr, nullptr); /* obtain a pointer on the current content
                                       of tinyfd_inputBox */
    if (lPointerInputBox)
        strcpy(lString, lPointerInputBox); /* preserve the current content of
                                              tinyfd_inputBox */
    p = tinyfd_inputBox(aTitle, "Enter hex rgb color (i.e. #f5ca20)",
                        lDefaultHexRGB);
...
static int dirExists(char const* aDirPath) {
    DIR* lDir;
    if (!aDirPath || !strlen(aDirPath)) return 0;
...
static int detectPresence(char const* aExecutable) {
    ...
    strcat(lTestedString, " 2>/dev/nullptr ");
    lIn = popen(lTestedString, "r");
    if ((fgets(lBuff, sizeof(lBuff), lIn) != nullptr) &&
        ...
        pclose(lIn);
                      ...
                      the top of the file*/
        if (lBuff[strlen(lBuff) - 1] == '\n') lBuff[strlen(lBuff) - 1] = '\0';
        lAllocatedCharString =
            realpath(lBuff, nullptr); /*same as canonicalize_file_name*/
...
static char*
getVersion(char const* aExecutable) /*version must be first numeral*/
{
    static char lBuff[MAX_PATH_OR_CMD];
...
static int* getMajorMinorPatch(char const* aExecutable) {
    ...
    if (!lArray[0] && !lArray[1] && !lArray[2]) return nullptr;
    return lArray;
}
...
static int tryCommand(char const* aCommand) {
    ...
    if (fgets(lBuff, sizeof(lBuff), lIn) == nullptr) { /* present */
        pclose(lIn);
        return 1;
    } else {
        pclose(lIn);
...
static char* dialogNameOnly(void) {
    ...
    if (lDialogName[0] == '*') {
        if (!tinyfd_allowCursesDialogs) {
            strcpy(lDialogName, "");
        } else if (tfd_isDarwin() &&
                   *strcpy(lDialogName, "/opt/local/bin/dialog") &&
...
int isDialogVersionBetter09b(void) {
    ...
    /*printf("lLetter %s\n", lLetter);*/
    lResult = (lMajor > 0) ||
              ((lMinor == 9) && (*lLetter == 'b') && (lDate >= 20031126));
    /*printf("lResult %d\n", lResult);*/
    return lResult;
}
    ...
    if (lTerminalName[0] == '*') {
        ...
        if (tfd_isDarwin()) {
            if (*strcpy(lTerminalName, "/opt/X11/bin/xterm") &&
                ...
                strcat(lTerminalName, " -fa 'DejaVu Sans Mono' -fs 10 -title "
                                      "tinyfiledialogs -e ");
                ...
                strcpy(lTerminalName, "");
                           ...
                           "xterm") /*good (small without parameters)*/
                   && detectPresence(lTerminalName)) {
            strcat(lTerminalName,
...
static int playPresent(void) /* play is part of sox */
{
    ...
    if (lPlayPresent < 0) {
        lPlayPresent =
            detectPresence("sox"); /*if sox is present, play is ready*/
    }
    return lPlayPresent;
}
...
static int playsoundPresent(void) /* playsound is part of pipewire */
{
    ...
    if (lPlaysoundPresent < 0) {
        ...
        if (lPlaysoundPresent &&
            !fileExists("/usr/share/sounds/freedesktop/stereo/bell.oga")) {
            lPlaysoundPresent = 0;
        }
    }
...
static int xmessagePresent(void) {
    ...
    if (lXmessagePresent < 0) {
        lXmessagePresent =
            detectPresence("xmessage"); /*if not tty,not on osxpath*/
    }
...
static int perlPresent(void) {
    ...
    if (lPerlPresent < 0) {
        ...
        if (lPerlPresent) {
            lIn = popen("perl -MNet::DBus -e "
                        "\"Net::DBus->session->get_service('org.freedesktop."
                        "Notifications')\" 2>&1",
                        "r");
            if (fgets(lBuff, sizeof(lBuff), lIn) == nullptr) {
                lPerlPresent = 2;
...
static int afplayPresent(void) {
    ...
    if (lAfplayPresent < 0) {
        ...
        if (lAfplayPresent) {
            lIn = popen("test -e /System/Library/Sounds/Ping.aiff || echo Ping",
                        "r");
            if (fgets(lBuff, sizeof(lBuff), lIn) == nullptr) {
                lAfplayPresent = 2;
            }
...
static int dunstPresent(void) {
    ...
    if (lDunstPresent < 0) {
        ...
        if (lDunstPresent) {
            lIn = popen("ps -e | grep dunst | grep -v grep",
                        "r"); /* add "| wc -l" to receive the number of lines */
            lTmp = fgets(lBuff, sizeof(lBuff), lIn);
            pclose(lIn);
...
int tfd_shellementaryPresent(void) {
    ...
    if (lShellementaryPresent < 0) {
        lShellementaryPresent = 0; /*detectPresence("shellementary");
                                      shellementary is not ready yet */
    }
...
int tfd_xpropPresent(void) {
    ...
    if (!lXpropReady &&
        lXpropDetected) { /* xwayland Debian issue reported by Kay F. Jahnke and
                             solved with his help */
    ...
    if (lZenity3Present < 0) {
        ...
        if (tfd_zenityPresent()) {
            ...
            if (fgets(lBuff, sizeof(lBuff), lIn) != nullptr) {
                ...
                } else if ((atoi(lBuff) == 2) &&
                           (atoi(strtok(lBuff, ".") + 2) >= 32)) {
                    lZenity3Present = 2;
                }
...
int tfd_kdialogPresent(void) {
    ...
    if (lKdialogPresent < 0) {
        ...
        if (!lDesktop ||
            (strcmp(lDesktop, "KDE") && strcmp(lDesktop, "lxqt"))) {
            if (tfd_zenityPresent()) {
                lKdialogPresent = 0;
...
static int osx9orBetter(void) {
    ...
    if (lOsx9orBetter < 0) {
        lOsx9orBetter = 0;
        lIn = popen(
            "osascript -e 'set osver to system version of (system info)'", "r");
        V = 0;
        if ((fgets(lBuff, sizeof(lBuff), lIn) != nullptr) &&
...
static int tkinter3Present(void) {
    ...
    return lTkinter3Present && graphicMode() &&
           !(tfd_isDarwin() && getenv("SSH_TTY"));
}
...
notify=dbus.Interface(notif,'org.freedesktop.Notifications');\nexcept:\n\tprint(0);\"";
    ...
    return lPythonDbusPresent && graphicMode() &&
           !(tfd_isDarwin() && getenv("SSH_TTY"));
}
...
void tinyfd_beep(void) {
    ...
    if (pactlPresent()) {
        ...
        strcpy(lDialogString,
               "thnum=$(pactl load-module module-sine frequency=440);sleep "
               ".3;pactl unload-module $thnum");
        ...
        /*strcpy( lDialogString , "timeout -k .3 .3 speaker-test --frequency 440
         * --test sine > /dev/tty" ) ;*/
        strcpy(lDialogString,
               "( speaker-test -t sine -f 440 > /dev/tty )& pid=$!;sleep .5; "
               "kill -9 $pid"); /*.3 was too short for mac g3*/
    } else if (ffplayPresent()) {
        strcpy(lDialogString,
               "ffplay -f lavfi -i sine=f=440:d=0.15 -autoexit -nodisp");
    } else if (playPresent()) /* play is part of sox */
    {
        ...
        strcpy(
            lDialogString,
            "playsound_simple /usr/share/sounds/freedesktop/stereo/bell.oga");
                                ...
                                yesnocancel */
{
    ...
    if (!aTitle || strcmp(aTitle, "tinyfd_query")) {
        lDialogString =
            (char*) malloc(MAX_PATH_OR_CMD + lTitleLen + lMessageLen);
    }
    ...
    if (osascriptPresent()) {
        ...
        strcat(lDialogString, " -e 'try' -e 'set {vButton} to {button "
                              "returned} of ( display dialog \"");
        if (aMessage && strlen(aMessage)) {
            strcat(lDialogString, aMessage);
        }
        strcat(lDialogString, "\" ");
 ...
 -e 'else' -e 'return 0' -e 'end if' ");
        ...
        if ((tfd_kdialogPresent() == 2) && tfd_xpropPresent()) {
            strcat(lDialogString,
                   " --attach=$(xprop -root 32x '\t$0' _NET_ACTIVE_WINDOW | "
                   "cut -f 2)"); /* contribution: Paul Rouget */
        }
        ...
        if (!strcmp("yesnocancel", aDialogType)) {
            strcat(lDialogString, "; x=$? ;if [ $x = 0 ] ;then echo 1;elif [ "
                                  "$x = 1 ] ;then echo 2;else echo 0;fi");
        } else {
            strcat(lDialogString, ";if [ $? = 0 ];then echo 1;else echo 0;fi");
        }
    } else if (tfd_zenityPresent() || tfd_matedialogPresent() ||
               tfd_shellementaryPresent() || tfd_qarmaPresent()) {
        if (tfd_zenityPresent()) {
            if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
            ...
            if ((tfd_zenity3Present() >= 4) && !getenv("SSH_TTY") &&
                tfd_xpropPresent()) {
                strcat(lDialogString,
                       " --attach=$(sleep .01;xprop -root 32x '\t$0' "
                       "_NET_ACTIVE_WINDOW | cut -f 2)"); /* contribution: Paul
                                                             Rouget */
            }
        ...
        if (tinyfd_silent) strcat(lDialogString, " 2>/dev/nullptr ");

        if (!strcmp("yesnocancel", aDialogType)) {
            strcat(lDialogString,
                   ");if [ $? = 1 ];then echo 0;elif [ $szAnswer = \"No\" "
                   "];then echo 2;else echo 1;fi");
        } else {
            strcat(lDialogString, ");if [ $? = 0 ];then echo 1;else echo 0;fi");
        }
    }
        ...
        if (aIconType &&
            (!strcmp("question", aIconType) || !strcmp("error", aIconType) ||
             !strcmp("warning", aIconType))) {
            strcat(lDialogString, aIconType);
        } else {
            strcat(lDialogString, "information");
        ...
        strcat(lDialogString, ");echo $?");
             ...
             !xdialogPresent() && tkinter3Present()) {
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
        ...
        strcat(lDialogString, " -S -c \"import tkinter;from tkinter import "
                              "messagebox;root=tkinter.Tk();root.withdraw();");
...
if res is False :\n\tprint(0)\n\
               ...
               !xdialogPresent() && tkinter2Present()) {
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
        ...
        strcat(lDialogString,
               " -S -c \"import "
               "Tkinter,tkMessageBox;root=Tkinter.Tk();root.withdraw();");
...
if res is False :\n\tprint 0\n\
               ...
               (!gdialogPresent() && !xdialogPresent() && xmessagePresent())) {
        if (gxmessagePresent()) {
            if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
        ...
        strcat(lDialogString, " ; echo $? ");
    } else if (xdialogPresent() || gdialogPresent() || dialogName() ||
               whiptailPresent()) {
        if (gdialogPresent()) {
            if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
...
tinyfdRes=$(cat /tmp/tinyfd.txt);echo $tinyfdBool$tinyfdRes");
            ...
            if (aDialogType && !strcmp("yesnocancel", aDialogType)) {
                ...
                if (lWasXterm) {
                    strcat(lDialogString, "then\n\techo 1\nelse\n\techo 0\nfi "
                                          ">/tmp/tinyfd.txt';cat "
                                          "/tmp/tinyfd.txt;rm /tmp/tinyfd.txt");
                } else {
                    strcat(lDialogString,
    ...
    } else if (!isTerminalRunning() && pythonDbusPresent() &&
        ...
        strcat(lDialogString,
               "notif=bus.get_object('org.freedesktop.Notifications','/org/"
    ...
    } else if (!isTerminalRunning() && (perlPresent() >= 2) &&
               !strcmp("ok", aDialogType)) {
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
...
'org.freedesktop.Notifications');");
        ...
        sprintf(lDialogString + strlen(lDialogString),
                "my \\$notificationId;\\$notificationId = "
                "\\$notificationsObject->Notify(shift, 0, '%s', '%s', '%s', "
                "[], {}, -1);\" ",
                aIconType ? aIconType : "", aTitle ? aTitle : "",
                aMessage ? aMessage : "");
    } else if (!isTerminalRunning() && notifysendPresent() &&
               !strcmp("ok", aDialogType)) {
...
'org.freedesktop.Notifications');");
        ...
        /* zenity 2.32 & 3.14 has the notification but with a bug: it doesnt
         * return from it */
    ...
    char const* aDefaultInput) /* "" , if nullptr it's a passwordBox */
{
    ...
    if (osascriptPresent()) {
        ...
        strcpy(lDialogString, gPython3Name);
        strcat(lDialogString,
               " -S -c \"import tkinter; from tkinter import "
               "simpledialog;root=tkinter.Tk();root.withdraw();");
        ...
        strcat(lDialogString,
               " -S -c \"import "
               "Tkinter,tkSimpleDialog;root=Tkinter.Tk();root.withdraw();");
...
frontmost of process \\\"Python\\\" to true' ''');");
        ...
        strcat(lDialogString, ");\nif res is None :\n\tprint 0");
        strcat(lDialogString, "\nelse :\n\tprint '1'+res\n\"");
    } else if (gdialogPresent() || xdialogPresent() || dialogName() ||
               whiptailPresent()) {
        if (gdialogPresent()) {
            if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
                strcpy(tinyfd_response, "gdialog");
        ...
        if (!xdialogPresent() && !gdialogPresent()) {
            ...
            if (!aDefaultInput && !lWasGdialog) {
                strcat(lDialogString, " (sometimes nothing, no blink nor star, "
                                      "is shown in text field)");
            }
            strcat(lDialogString, "\" ");
        }
...
char* tinyfd_saveFileDialog(
    ...
    char const* aDefaultPathAndOrFile,    /* nullptr or "" , ends with / to set
                                             only a    directory */
    int aNumOfFilterPatterns,             /* 0 */
    char const* const* aFilterPatterns,   /* nullptr or {"*.txt","*.doc"} */
    char const* aSingleFilterDescription) /* nullptr or "text files" */
{
    ...
    if (osascriptPresent()) {
        ...
        strcpy(lDialogString, gPython3Name);
        strcat(lDialogString, " -S -c \"import tkinter;from tkinter import "
                              "filedialog;root=tkinter.Tk();root.withdraw();");
        ...
        strcat(lDialogString,
               " -S -c \"import "
               "Tkinter,tkFileDialog;root=Tkinter.Tk();root.withdraw();");
 ...
 frontmost of process \\\"Python\\\" to true' ''');");
        ...
        if (!xdialogPresent() && !gdialogPresent()) {
            strcat(lDialogString, "--backtitle \"");
            strcat(lDialogString, "tab: focus | /: populate | spacebar: fill "
                                  "text field | ok: TEXT FIELD ONLY");
            strcat(lDialogString, "\" ");
        }
        ...
        if (lWasGraphicDialog) {
            ...
            if (lWasXterm) {
                strcat(lDialogString, "2>/tmp/tinyfd.txt';cat "
                                      "/tmp/tinyfd.txt;rm /tmp/tinyfd.txt");
            } else {
        ...
        if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
            return tinyfd_inputBox(aTitle, nullptr, nullptr);
        }
        strcpy(lBuff, "Save file in ");
        strcat(lBuff, getCurDir());
        lPointerInputBox = tinyfd_inputBox(
    ...
    int aAllowMultipleSelects)            /* 0 or 1 */
{
    ...
    if (osascriptPresent()) {
        ...
        if (aAllowMultipleSelects) {
            ...
            strcat(lDialogString, "-e 'set mystring to mystring & POSIX path "
                                  "of item i of mylist' ");
...
\n\tprint lFilesString[:-1]\n\"");
        ...
        if (lPointerInputBox)
            strcpy(lPointerInputBox,
                   lDialogString); /* restore its previous content to
                                      tinyfd_inputBox */
        if (!fileExists(lBuff)) {
            free(lBuff);
            lBuff = nullptr;
            ...
            lBuff =
                (char*) (realloc(lBuff, (strlen(lBuff) + 1) * sizeof(char)));
        }
        return lBuff;
    }
    ...
    lBuff[0] = '\0';
    p = lBuff;
    while (fgets(p, sizeof(lBuff), lIn) != nullptr) {
        p += strlen(p);
    }
    pclose(lIn);
...
to set mycolor to choose color default color {");
        ...
        strcat(lDialogString, lTmp);
        strcat(lDialogString, "}' ");
        strcat(lDialogString, "-e 'set mystring to ((item 1 of mycolor) div "
                              "256 as integer) as string' ");
        strcat(lDialogString,
               "-e 'repeat with i from 2 to the count of mycolor' ");
        strcat(lDialogString, "-e 'set mystring to mystring & \" \" & ((item i "
                              "of mycolor) div 256 as integer) as string' ");
        strcat(lDialogString, "-e 'end repeat' ");
        strcat(lDialogString, "-e 'mystring' ");
        strcat(lDialogString, "-e 'on error number -128' ");
        strcat(lDialogString, "-e 'end try'");
        ...
        sprintf(lDialogString + strlen(lDialogString), " --color=%s",
                lDefaultHexRGB);
        ...
        sprintf(lDialogString + strlen(lDialogString), " --init-color=%s",
                lDefaultHexRGB);
        if (aTitle && strlen(aTitle)) {
            strcat(lDialogString, " --title=\"");
            strcat(lDialogString, aTitle);
    ...
    (defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L) ||              \
    defined(__clang__)
        sprintf(lTmp, "%hhu %hhu %hhu", lDefaultRGB[0], lDefaultRGB[1],
                lDefaultRGB[2]);
#else
        sprintf(lTmp, "%hu %hu %hu", lDefaultRGB[0], lDefaultRGB[1],
                lDefaultRGB[2]);
#endif
        strcat(lDialogString, lTmp);
        ...
        strcat(lDialogString,
               " -S -c \"import tkinter;from tkinter import "
               "colorchooser;root=tkinter.Tk();root.withdraw();");
...
\nif res[1] is not None:\n\tprint(res[1])\"");
        ...
        strcat(lDialogString,
               " -S -c \"import "
               "Tkinter,tkColorChooser;root=Tkinter.Tk();root.withdraw();");
...
\nif res[1] is not None:\n\tprint res[1]\"");
    ...
    (defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L) ||              \
    defined(__clang__)
            sscanf(lBuff, "rgb(%hhu,%hhu,%hhu", &aoResultRGB[0],
            ...
            aoResultRGB[2] = (unsigned char) strtol(lTmp2 + 1, nullptr, 10);
#endif
    ...
    (defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L) ||              \
    defined(__clang__)
            sscanf(lBuff, "rgba(%hhu,%hhu,%hhu", &aoResultRGB[0],
                   &aoResultRGB[1], &aoResultRGB[2]);
#else
    ...
    (defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L) ||              \
    defined(__clang__)
        sscanf(lBuff, "%hhu %hhu %hhu", &aoResultRGB[0], &aoResultRGB[1],
               &aoResultRGB[2]);
#else
...
void tfd_messageBox(char const* aTitle, char const* aMessage,
                    char const* aDialogType, char const* aIconType,
                    int* aiDefaultButton) {
    *aiDefaultButton = tinyfd_messageBox(aTitle, aMessage, aDialogType,
                                         aIconType, *aiDefaultButton);
}

void tfd_inputBox(char const* aTitle, char const* aMessage,
                  char** aiDefaultInput) {
    char* lReturnedInput;
    if (!strcmp(*aiDefaultInput, "nullptr"))
        lReturnedInput = tinyfd_inputBox(aTitle, aMessage, nullptr);
    else lReturnedInput = tinyfd_inputBox(aTitle, aMessage, *aiDefaultInput);

    if (lReturnedInput) strcpy(*aiDefaultInput, lReturnedInput);
    else strcpy(*aiDefaultInput, "nullptr");
}

void tfd_saveFileDialog(char const* aTitle, char** aiDefaultPathAndFile,
    ...
    lSavefile = tinyfd_saveFileDialog(aTitle, *aiDefaultPathAndFile,
                                      *aNumOfFilterPatterns, aFilterPatterns,
                                      aSingleFilterDescription);
    if (lSavefile) strcpy(*aiDefaultPathAndFile, lSavefile);
    else strcpy(*aiDefaultPathAndFile, "nullptr");
}

void tfd_openFileDialog(char const* aTitle, char** aiDefaultPathAndFile,
                        int const* aNumOfFilterPatterns,
                        char const* const* aFilterPatterns,
                        char const* aSingleFilterDescription,
    ...
    lOpenfile = tinyfd_openFileDialog(
        aTitle, *aiDefaultPathAndFile, *aNumOfFilterPatterns, aFilterPatterns,
...
void tfd_selectFolderDialog(char const* aTitle, char** aiDefaultPath) {
    ...
    else strcpy(*aiDefaultPath, "nullptr");
}
...
void tfd_colorChooser(char const* aTitle, char** aiDefaultHexRGB) {
    ...
    char* lChosenColor;
    lChosenColor =
        tinyfd_colorChooser(aTitle, *aiDefaultHexRGB, aDefaultRGB, aoResultRGB);
    if (lChosenColor) strcpy(*aiDefaultHexRGB, lChosenColor);
    else strcpy(*aiDefaultHexRGB, "nullptr");
}
...
lTmp = tinyfd_inputBox(
                "a password box", "your password will be revealed", nullptr);
